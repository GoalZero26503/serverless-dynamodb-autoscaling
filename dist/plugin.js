...
class AWSDBAutoScaling {
    ...
    resources(table, index, config) {
        ...
        const resources = [new role_1.default(options)];
    ...
    getPolicyAndTarget(options, data, read) {
        return [
            new policy_1.default(options, read, data.usage * 100, data.scaleIn || 60, data.scaleOut || 60),
    ...
    process() {
        this.serverless.service.custom.capacities
            ...
            .forEach((config) => this.normalize(config.table).forEach((table) => this.generate(table, config).forEach((resource) => _.merge(this.serverless.service.provider.compiledCloudFormationTemplate
            .Resources, resource))));
    }
    beforeDeployResources() {
        return Promise.resolve()
            ...
            .catch((err) => this.serverless.cli.log(util.format(text.CLI_SKIP, err.message)));
    }
}
